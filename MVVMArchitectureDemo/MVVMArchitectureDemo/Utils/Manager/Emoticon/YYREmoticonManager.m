...
#import "YYREmoticonManager.h"

@implementation YYREmoticonManager
...
+ (NSArray<YYREmoticonGroup *> *)emoticonGroups{
    ...
    dispatch_once(&onceToken, ^{
        ...
        groups = (NSMutableArray *)[YYREmoticonGroup modelArrayWithJSON:packages];
        NSMutableDictionary *groupDic = [NSMutableDictionary new];
        for (int i = 0, max = (int)groups.count; i < max; i++) {
            YYREmoticonGroup *group = groups[i];
        ...
        for (NSString *path in additionals) {
            YYREmoticonGroup *group = groupDic[path];
            if (!group) continue;
            ...
            YYREmoticonGroup *addGroup = [YYREmoticonGroup modelWithJSON:infoJSON];
            if (addGroup.emoticons.count) {
                for (YYREmoticon *emoticon in addGroup.emoticons) {
