...
/// 允许最大选中照片数
static CGFloat const YYRMaxImagesCount = 8;

@implementation YYRPhotoManager
...
+ (void)fetchPhotosFromCamera:(UIViewController *)viewController allowCrop:(BOOL)allowCrop completion:(void (^)(UIImage *, id))completion {
    viewController = (viewController == nil)?[YYRControllerHelper topViewController]:viewController;
    /// 检查授权
    AVAuthorizationStatus authStatus = [AVCaptureDevice authorizationStatusForMediaType:AVMediaTypeVideo];
    if ((authStatus == AVAuthorizationStatusRestricted || authStatus == AVAuthorizationStatusDenied) && iOS7Later) {
        // 无相机权限 做一个友好的提示 ，这个只是针对 iOS8+
        [NSObject yyr_showAlertViewWithTitle:@"无法访问你的相机" message:@"请在iPhone的“设置-隐私-相机”选项中，允许轻空访问你相机" confirmTitle:@"设置" cancelTitle:@"取消" confirmAction:^{
        ...
        [NSObject yyr_showAlertViewWithTitle:@"无法访问你的相册" message:@"请在iPhone的“设置-隐私-相册”选项中，允许轻空访问你相册" confirmTitle:@"设置" cancelTitle:@"取消" confirmAction:^{
...
//        }];
        ...
        if ([UIImagePickerController isSourceTypeAvailable: UIImagePickerControllerSourceTypeCamera]) {
            ...
            title = @"无法打开你的相机！";
#endif
            [NSObject yyr_showAlertViewWithTitle:title message:nil confirmTitle:@"我知道了"];
        }
    }
}
...
+ (void)configureImagePicker:(TZImagePickerController *)imagePicker {
    ...
    /// 圆形裁剪size
    imagePicker.circleCropRadius = (YYR_SCREEN_MIN_LENGTH - 60);
    /// 配置cropView
    ...
    NSInteger left = 0;
    NSInteger widthHeight = YYR_SCREEN_WIDTH - 2 * left;
    NSInteger top = (YYR_SCREEN_HEIGHT - widthHeight) / 2;
    /// 裁剪尺寸
...
+ (void) accessApplicationSetting:(BOOL)isAblum {
    if (iOS8Later) {
        if (![[UIApplication sharedApplication] openURL:[NSURL URLWithString:UIApplicationOpenSettingsURLString]]) {
            [NSObject yyr_showAlertViewWithTitle:@"十分抱歉" message:@"无法跳转到隐私设置页面，请手动前往设置页面，谢谢" confirmTitle:@"确定"];
        }
    }
}
...
+ (NSInteger)colunmNumber {
    return (YYR_IS_IPHONE_5||YYR_IS_IPHONE_4_OR_LESS) ? 3 : 4;
}

/// 允许最大选择
+ (NSInteger)maxImagesCount {
    return YYRMaxImagesCount;
}
