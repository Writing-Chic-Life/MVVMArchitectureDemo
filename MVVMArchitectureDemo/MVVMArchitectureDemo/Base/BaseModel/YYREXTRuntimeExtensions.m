...
yyr_propertyAttributes *yyr_copyPropertyAttributes (objc_property_t property) {
   ...
   yyr_propertyAttributes *attributes = calloc(1, sizeof(yyr_propertyAttributes) + typeLength + 1);
    if (!attributes) {
        fprintf(stderr, "ERROR: Could not allocateyyr_propertyAttributes structure for attribute string \"%s\" for property %s\n", attrString, property_getName(property));
        return NULL;
    }
    ...
    while (next && *next == ',') {
        ...
        switch (flag) {
            ...
            case 'C':
                attributes->memoryManagementPolicy =yyr_propertyMemoryManagementPolicyCopy;
                break;
                
            case '&':
                attributes->memoryManagementPolicy =yyr_propertyMemoryManagementPolicyRetain;
                break;
