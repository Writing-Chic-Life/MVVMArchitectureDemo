...
#import "YYRObject.h"
#import "YYRReflection.h"
#import "YYREXTRuntimeExtensions.h"

// Used to cache the reflection performed in +propertyKeys.
static void *YYRObjectCachedPropertyKeysKey = &YYRObjectCachedPropertyKeysKey;
...
+ (NSSet *)propertyKeys {
    NSSet *cachedKeys = objc_getAssociatedObject(self, YYRObjectCachedPropertyKeysKey);
    if (cachedKeys != nil) return cachedKeys;
    
    NSMutableSet *keys = [NSMutableSet set];
    
    [self enumeratePropertiesUsingBlock:^(objc_property_t property, BOOL *stop) {
        yyr_propertyAttributes *attributes = yyr_copyPropertyAttributes(property);
        @onExit {
            free(attributes);
        };
    ...
    objc_setAssociatedObject(self, YYRObjectCachedPropertyKeysKey, keys, OBJC_ASSOCIATION_COPY);
...
+ (void)enumeratePropertiesUsingBlock:(void (^)(objc_property_t property, BOOL *stop))block {
    ...
    while (!stop && ![cls isEqual:YYRObject.class]) {
        unsigned count = 0;
...
#pragma mark Merging
- (void)mergeValueForKey:(NSString *)key fromModel:(YYRObject *)model {
    NSParameterAssert(key != nil);
    
    SEL selector = YYRSelectorWithCapitalizedKeyPattern("merge", key, "FromModel:");
...
- (void)mergeValuesForKeysFromModel:(YYRObject *)model {
