...
@interface YYRNavigationController ()
...
@implementation YYRNavigationController
...
- (void)_setupNavigationBarBottomLine {
    ...
    UIImageView *navSystemLine = [[UIImageView alloc] initWithFrame:CGRectMake(0, self.navigationBar.yyr_height - navSystemLineH, YYR_SCREEN_WIDTH, navSystemLineH)];
    navSystemLine.backgroundColor = YYRColor(223.0f, 223.0f, 221.0f);
...
+ (void) _setupNavigationBarTheme {
    ...
    textAttrs[NSFontAttributeName] = YYRMediumFont(18.0f);
...
+ (void)_setupBarButtonItemTheme {
    ...
    NSMutableDictionary *textAttrs = [NSMutableDictionary dictionary];
    textAttrs[NSForegroundColorAttributeName] = [UIColor whiteColor];
    textAttrs[NSFontAttributeName] = YYRRegularFont(fontSize);
    NSShadow *shadow = [[NSShadow alloc] init];
    shadow.shadowOffset =  CGSizeZero;
    textAttrs[NSShadowAttributeName] = shadow;
...
- (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated {
    ...
    if (self.viewControllers.count > 0){
        ...
        ///  Fixed: [[self topViewController] navigationItem].title 这样来获取title 而不是[[self topViewController] title]
        title = [[self topViewController] navigationItem].title ? : @"返回";
        
        /// 2.判断要被Push的控制器（B）是否是 MHViewController ，
        if ([viewController isKindOfClass:[YYRViewController class]]) {
            
            YYRViewModel *viewModel = [(YYRViewController *)viewController viewModel];
        ...
        viewController.navigationItem.leftBarButtonItem = [UIBarButtonItem yyr_backItemWithTitle:title imageName:@"barbuttonicon_back_15x30" target:self action:@selector(_back)];
    }
    // push
