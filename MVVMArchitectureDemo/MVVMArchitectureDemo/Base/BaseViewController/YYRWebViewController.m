...
#import "YYRWebViewController.h"
#import "FBKVOController+YYRExtension.h"
#import "UIScrollView+YYRRefresh.h"

/// KVO 监听的属性
/// 加载情况
static NSString * const YYRWebViewKVOLoading = @"loading";
/// 文章标题
static NSString * const YYRWebViewKVOTitle = @"title";
/// 进度
static NSString * const YYRWebViewKVOEstimatedProgress = @"estimatedProgress";

@interface YYRWebViewController ()
...
/// viewModel
@property (nonatomic, strong, readonly) YYRWebViewModel *viewModel;

@end

@implementation YYRWebViewController {
...
- (void)viewDidLoad {
    ...
    if (!(YYRIOSVersion>=9.0)) [[NSUserDefaults standardUserDefaults] registerDefaults:@{@"userAgent":userAgent}];
    ...
    WKWebView *webView = [[WKWebView alloc] initWithFrame:YYR_SCREEN_BOUNDS configuration:configuration];
    webView.navigationDelegate = self;
    webView.UIDelegate = self;
    
    if ((YYRIOSVersion >= 9.0)) webView.customUserAgent = userAgent;
    ...
    [_KVOController yyr_observe:self.webView keyPath:YYRWebViewKVOTitle block:^(id  _Nullable observer, id  _Nonnull object, NSDictionary<NSString *,id> * _Nonnull change) {
        @strongify(self);
    ...
    [_KVOController yyr_observe:self.webView keyPath:YYRWebViewKVOLoading block:^(id  _Nullable observer, id  _Nonnull object, NSDictionary<NSString *,id> * _Nonnull change) {
        NSLog(@"--- webView is loading ---");
    }];
    
    [_KVOController yyr_observe:self.webView keyPath:YYRWebViewKVOEstimatedProgress block:^(id  _Nullable observer, id  _Nonnull object, NSDictionary<NSString *,id> * _Nonnull change) {
        @strongify(self);
    ...
    if(self.viewModel.shouldPullDownToRefresh) {
        [self.webView.scrollView yyr_addHeaderRefresh:^(MJRefreshNormalHeader *header) {
            @strongify(self);
            [self.webView reload];
        }];
    ...
    if (@available(iOS 11.0, *)) {
        YYRAdjustsScrollViewInsets_Never(webView.scrollView);
    }
}
...
- (UIEdgeInsets)contentInset {
    return UIEdgeInsetsMake(YYR_APPLICATION_TOP_BAR_HEIGHT, 0, 0, 0);
}
...
- (void)webView:(WKWebView *)webView runJavaScriptAlertPanelWithMessage:(NSString *)message initiatedByFrame:(WKFrameInfo *)frame completionHandler:(void (^)(void))completionHandler {
    [NSObject yyr_showAlertViewWithTitle:nil message:message confirmTitle:@"我知道了"];
    completionHandler();
}
...
- (UIProgressView *)progressView {
    if (!_progressView) {
        CGFloat progressViewW = YYR_SCREEN_WIDTH;
        ...
        progressView.progressTintColor = YYR_MAIN_TINTCOLOR;
...
- (UIBarButtonItem *)backItem {
    if (_backItem == nil) {
        _backItem = [UIBarButtonItem yyr_backItemWithTitle:@"返回" imageName:@"barbuttonicon_back_15x30" target:self action:@selector(_backItemDidClicked)];
    }
    return _backItem;
}
...
- (UIBarButtonItem *)closeItem {
    if (!_closeItem) {
        _closeItem = [UIBarButtonItem yyr_systemItemWithTitle:@"关闭" titleColor:nil imageName:nil target:self selector:@selector(_closeItemDidClicked) textType:YES];
    }
    return _closeItem;
}
