...
#import "YYRTableViewController.h"
#import "UIScrollView+YYRRefresh.h"

@interface YYRTableViewController ()

/// tableView
@property (nonatomic, readwrite, weak)   YYRTableView *tableView;
/// contentInset defaul is (64 , 0 , 0 , 0)
@property (nonatomic, readwrite, assign) UIEdgeInsets contentInset;
/// 视图模型
@property (nonatomic, readonly, strong) YYRTableViewModel *viewModel;

@end

@implementation YYRTableViewController
...
/// init
- (instancetype)initWithViewModel:(YYRTableViewModel *)viewModel {
...
- (void)_su_setupSubViews {
    ...
    YYRTableView *tableView = [[YYRTableView alloc] initWithFrame:[UIScreen mainScreen].bounds style:self.viewModel.style];
    tableView.backgroundColor = YYR_MAIN_BACKGROUNDCOLOR;
    ...
    if (self.viewModel.shouldPullDownToRefresh) {
        /// 下拉刷新
        @weakify(self);
        [self.tableView yyr_addHeaderRefresh:^(MJRefreshNormalHeader *header) {
            /// 加载下拉刷新的数据
            @strongify(self);
    ...
    if (self.viewModel.shouldPullUpToLoadMore) {
        /// 上拉加载
        @weakify(self);
        [self.tableView yyr_addFooterRefresh:^(MJRefreshAutoNormalFooter *footer) {
            /// 加载上拉刷新的数据
            @strongify(self);
    ...
    if (@available(iOS 11.0, *)) {
        ...
        YYRAdjustsScrollViewInsets_Never(tableView);
