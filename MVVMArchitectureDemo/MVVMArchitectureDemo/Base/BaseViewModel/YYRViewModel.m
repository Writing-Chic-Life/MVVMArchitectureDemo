...
NSString *const YYRViewModelIDKey = @"YYRViewModelIDKey";
/// 传递导航栏title的key：例如 导航栏的title...
NSString *const YYRViewModelTitleKey = @"YYRViewModelTitleKey";
/// 传递数据模型的key：例如 商品模型的传递 用户模型的传递...
NSString *const YYRViewModelUtilKey = @"YYRViewModelUtilKey";
/// 传递webView Request的key：例如 webView request...
NSString *const YYRViewModelRequestKey = @"YYRViewModelRequestKey";

@interface YYRViewModel ()

/// 整个应用的服务层 The `services` parameter in `-initWithServices:params` method.
@property (nonatomic, strong, readwrite) id<YYRViewModelServices> services;
...
- (instancetype)initWithServices:(id<YYRViewModelServices>)services params:(NSDictionary *)params {
    ...
    if (self) {
        ...
        self.title = params[YYRViewModelTitleKey];
        /// 赋值
